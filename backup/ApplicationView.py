...
import CellularAutomatonView as cav
...
class ApplicationView(GraphicalUserInterface):
    ...
    def __build(self) -> None:
        ...
        self.button_anim = Button(self.frame_right, text="Start animation", command=self.draw)
        ...
        self.scrollbar_vertical_canvas = Scrollbar(self.frame_canvas, orient="vertical", command=self.canvas.yview)
        self.scrollbar_vertical_canvas.grid(row=0, column=1, sticky='ns')
        self.canvas.configure(yscrollcommand=self.scrollbar_vertical_canvas.set)

        self.scrollbar_horizontal_canvas = Scrollbar(self.frame_canvas, orient="horizontal", command=self.canvas.xview)
        self.scrollbar_horizontal_canvas.grid(row=1, column=0, sticky='ew')
        self.canvas.configure(xscrollcommand=self.scrollbar_horizontal_canvas.set)
        """
    ...
    def __show_cellular_automaton_menu(self) -> None:
        ...
        if self.is_ca_menu_exists:
            return

        self.is_ca_menu_exists = True

        self.cellularAutomatonView = cav.CellularAutomatonView(self)
        self.ca_menu = self.cellularAutomatonView.mainWindow
        
        """self.ca_menu = Toplevel(self.root)
    ...
    """
    def __close_root(self) -> None:
        if self.root.winfo_exists():
            self.root.destroy()

    def __close_ca_menu(self) -> None:
        if self.ca_menu.winfo_exists():
            self.ca_menu.destroy()
    """